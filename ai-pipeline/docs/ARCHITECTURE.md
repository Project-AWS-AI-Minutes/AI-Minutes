...
### 🔄 데이터 파이프라인 흐름
1. **[FE ➡️ S3]** 클라이언트(AA)가 회의 음성 파일을 AWS S3 버킷에 직접 업로드 (Presigned URL 활용 권장).
...
4. **[AI 서버 폴링]** SA 파이썬 엔진은 백그라운드에서 주기적으로 SQS 큐를 확인(Polling)하다가 메시지를 수신.
5. **[AI Processing]** - AWS Transcribe API를 호출하여 STT 변환 진행 (상태: `TRANSCRIBING`).
   - 변환된 원본 텍스트를 LLM에 전달하여 요약 (상태: `SUMMARIZING`).
   - LLM에 프롬프트를 주입하여 담당자별 To-Do 데이터를 JSON으로 추출 (상태: `TODO_EXTRACTING`).
6. **[AI ➡️ Core]** AI 처리가 완료되면 SA 엔진이 Core API(웹훅 형태)를 호출하여 DB의 최종 상태(`COMPLETED`) 및 결과 데이터 업데이트.
...
3. **회의 요약 (`meeting_summary`):** FK(`meeting_id`), 핵심 요약 텍스트.
4. **할 일 추출 결과 (`meeting_todo`):** FK(`meeting_id`), 담당자명(`assignee`), 할 일 내용(`task_description`), 마감일.

## 3. 핵심 시스템 컴포넌트
* **Frontend (AA):** React / Node.js (상태값 기반 폴링 렌더링)
* **Core API (TA):** Spring Boot (REST API, DB 트랜잭션 관리, SQS Producer)
* **AI Engine (SA):** Python 3.12 (Boto3, LLM 연동, SQS Consumer / Worker)
* **Infrastructure:** AWS S3 (오디오 저장), AWS SQS (메시지 브로커), AWS Transcribe (음성 인식)
