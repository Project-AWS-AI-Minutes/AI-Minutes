...
- [ ] SQS 메시지 파싱 및 검증 (수신된 JSON 데이터 구조 확인)
...
- [ ] 프롬프트 엔지니어링 1: 원본 텍스트 기반 핵심 회의록 요약 프롬프트 작성
- [ ] 프롬프트 엔지니어링 2: 요약본 기반 담당자별 To-Do List(할 일) 추출 프롬프트 작성
  - *Point:* 반드시 정형화된 JSON 형태로 응답받도록 LLM 파라미터(Response Format) 설정
...
- [ ] 예외 처리(Try-Except) 및 에러 상태 로깅: 작업 실패 시 DB 상태를 `[FAILED]` 등으로 변경하고 큐(Queue)에 메시지 반환 혹은 데드레터큐(DLQ)로 이동