...
| AI Processing Service | SQS 메시지 소비, Transcribe/Amazon Bedrock (Claude 3.5 Sonnet) 호출, 결과 저장 |
...
8. AI Processing Service는 AWS Transcribe와 Amazon Bedrock (Claude 3.5 Sonnet)를 호출해 transcript, 요약, 개인별 To-Do를 생성한다.
9. 처리 결과는 RDS에 저장되고 회의 상태는 `COMPLETED` 또는 `FAILED`로 갱신된다.
10. 사용자는 Frontend를 통해 Archive 또는 상세 화면에서 결과를 조회한다.

### 2.3 요청 흐름 요약
`사용자 브라우저 → ALB → ECS Frontend → Core API → S3 / SQS → AI Processing Service → AWS Transcribe / Amazon Bedrock (Claude 3.5 Sonnet) → RDS`
...
| `BEDROCK_MODEL_ID` | Amazon Bedrock (Claude 3.5 Sonnet) 모델 ID |
...
6. AI Processing Service는 AWS 내부 네트워크에서 AWS Transcribe 및 Amazon Bedrock (Claude 3.5 Sonnet)에 접근한다.
7. RDS는 Private Subnet에 배치되어 ECS 서비스에서만 접근 가능하다.

### 5.3 AI 호출 구조
- AI Processing Service → AWS Transcribe
- AI Processing Service → Amazon Bedrock (Claude 3.5 Sonnet) API
