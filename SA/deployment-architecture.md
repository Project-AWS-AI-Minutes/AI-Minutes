...
- **배포 컨트롤러:** ECS Rolling Update (Direct Service Update)
- **보안/인증:** IAM OIDC (OpenID Connect) & 최소 권한 원칙(Least Privilege)

## 2. CI/CD 파이프라인 흐름

`GitHub Push -> GitHub Actions 트리거 -> AWS OIDC 연동 인증 -> Docker 이미지 빌드 및 ECR 업로드 -> ECS 서비스 업데이트 및 태스크 정의 등록 -> 신규 Task 실행 및 헬스체크 -> 이전 Task 안정적 종료 -> 배포 완료`
...
5. ECS가 새로운 버전의 Task Definition을 수신하여 새 컨테이너를 실행합니다.
6. 새 컨테이너가 정상적으로 실행(`Running`)되면, ECS가 자동으로 이전 버전의 컨테이너를 중지시킵니다. (롤링 업데이트 무중단 배포)
